...
let emf, spiritBox, fingerprints, orbs, writing, freezing
...
window.addEventListener("onWidgetLoad", function (obj) {
	...
	config.nameStrings = {
		noNameString: fieldData["noNameString"] ? fieldData["noNameString"] : "A New Ghostie",
		ghostNameString: fieldData["ghostNameString"] ? fieldData["ghostNameString"] : "Name: [name]"
	}
	config.optionalObj = {
		noOptionalString: fieldData["noOptionalObjectivesMessage"]
	}
	config.conclusionStrings = {
		zeroEvidenceConclusionString: fieldData["zeroEvidenceConclusionString"] ? fieldData["zeroEvidenceConclusionString"] : "Waiting for Evidence",
		oneEvidenceConclusionString: fieldData["oneEvidenceConclusionString"] ? fieldData["oneEvidenceConclusionString"] : "Not sure yet...",
		tooMuchEvidence: fieldData["impossibleConclusionString"] ? fieldData["impossibleConclusionString"] : "Too Much Evidence"
	}
	config.ghosts = [
		{
			type: "Banshee",
...
window.addEventListener("onEventReceived", function (obj) {
	...
	let i = badges.findIndex(
		(x) => x.type === "moderator" || x.type === "broadcaster" || (config.allowVIPS && config.vipCommandAccess && x.type === "vip") || data.displayName.toLowerCase() === "glitchedmythos"
	)
	if (i == -1) {
...
let checkEvidenceGhostMatch = () => {
	...
	// 1  Piece of Evidence
		...
		ghostGuessString = ghostPossibilityStrings.length == 0 ? "UH OH... no match?!" : ghostPossibilities[0].conclusion
	} // Too much evidence
	else {
		if (greyOutInvalidEvidence) {
			removeAllImpossibleCSS()
...
let createGhostConclusionString = (conclusionString, ghostType) => {
	return conclusionString ? conclusionString : `It's <strong style="font-size:21px;color:#77dd77">${ghostType}</strong>!`
}
